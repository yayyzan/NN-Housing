...
class SigmoidLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        sig = 1 / (1 + np.exp(-1 * self._cache_current))
        sig_prime = (sig) * (1 - sig)
        return sig_prime * grad_z
...
class ReluLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        relu_prime = np.where(self._cache_current <= 0, 0, 1)
        return relu_prime * grad_z
...
class LinearLayer(Layer):
    ...
    def forward(self, x):
        ...
        res = np.matmul(x, self._W) + self._b
        self._cache_current = x
        return res
    ...
    def backward(self, grad_z):
        ...
        self._grad_b_current = np.matmul(np.ones(np.shape(grad_z)[0]), grad_z)
...
class MultiLayerNetwork(object):
    ...
    def __init__(self, input_dim, neurons, activations):
        ...
        self._layers = []
        input_output = [input_dim] + self.neurons

        for i in range(len(input_output) - 1):
            activation = activations[i]
            self._layers.append(LinearLayer(input_output[i], input_output[i + 1]))
    ...
    def forward(self, x):
        ...
        for layer in self._layers:
            next_layer = layer(current_layer)
    ...
    def backward(self, grad_z):
        ...
        for layer in reversed(self._layers):
            grad_next = layer.backward(grad_current)
...
class Trainer(object):
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for _ in range(self.nb_epoch):
            ...
            output_target_batches = np.split(target_dataset, len(input_dataset)/self.batch_size)
            for batch in range(len(input_batches)):
                y_i = self.network(input_batches[batch])
                y_hat = output_target_batches[batch]
                self._loss_layer(y_i, y_hat)
    ...
    def eval_loss(self, input_dataset, target_dataset):
        ...
        return self._loss_layer(self.network(input_dataset), target_dataset)
